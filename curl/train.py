...
def run_task(vv, log_dir=None, exp_name=None):
    ...
    if vv['is_eval']:
        if vv['convert_two_arms_to_one_arm']:
            main_eval_convert_two_arms_to_one(vv_to_args(updated_vv))
        elif vv['eval_over_five_seeds']:
...
def get_info_stats(infos):
    ...
    for key in infos[0][0].keys():
        ...
        # stat_dict[key + '_sum'] = np.sum(np.array(stat_dict_all[key])) # caused wandb logging issues with info_normalized_performance_final (ran 3M but only logged 1M)
    return stat_dict
...
def main_eval_convert_two_arms_to_one(args):
    ...
    for target_ep_index in range(len(two_arms_data_configs)):
        ...
        print(f'Ep {target_ep_index}, info_normalized_performance_final: {ep_normalized_perf[-1]}')
...
def main_eval(args):
    ...
    for episode in range(50):
        ...
        if env.action_mode == 'pickerpickandplace':
            env.start_record()
        rewards = []
        ep_normalized_perf = []
        ...
        while not done:
            ...
            ep_normalized_perf.append(info['normalized_performance'])
        if env.action_mode == 'pickerpickandplace':
            env.end_record(video_path=os.path.join(video_dir, f'ep_{episode}_pickandplace_{ep_normalized_perf[-1]}.gif'))
        else:
            ...
            if step > 0:
                if step % args.log_interval == 0:
                    ...
                    for key, val in get_info_stats([ep_info]).items():
                        ...
                        # if args.wandb:
                        #     wandb.log({f'train/info_{key}': val, 'num_timesteps': step})
                    L.dump(step)
                start_time = time.time()
            if step % args.log_interval == 0:
