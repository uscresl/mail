...
class RunSettings(MasterClass):
    ...
    def get_config_file(self) -> str:
        ...
        :return: The location to a config file that holds whatever information
            about the project.
        """
        return osp.join(self.working_dir, "goal_prox_il/config.yaml")
    ...
    def create_runner(self, add_args={}, ray_create=False) -> rlf.Runner:
        ...
        # Setup environment
        if args.env_name == 'DryCloth' or args.env_name == 'ClothFold':
            from softgym.registered_env import env_arg_dict
            from sb3.utils import update_env_kwargs, str2bool
            from goal_prox_il_softgym_envs.env import SoftGymEnvSB3
            ...
            internal_args['seed']= args.seed
            internal_args['scale_reward'] = reward_scales[env_name]
            internal_args['clip_obs'] =  None
            internal_args['env_kwargs'] =  env_arg_dict[env_name]
            internal_args['env_kwargs_render'] = True
            internal_args['env_kwargs_camera_name'] = 'default_camera'
            internal_args['env_kwargs_observation_mode'] = 'cam_rgb'
            internal_args['env_kwargs_num_variations'] = 100
            internal_args['env_kwargs_env_image_size'] = 32
            internal_args['env_kwargs_num_picker'] = 1
            internal_args['action_mode'] = 'pickerpickandplace'
            internal_args['action_repeat'] = 1
            internal_args['horizon'] = 3

            internal_args = update_env_kwargs(internal_args)  # Update env_kwargs

            not_imaged_based = internal_args['env_kwargs']['observation_mode'] not in ['cam_rgb', 'cam_rgb_key_point', 'depth_key_point']
            symbolic = not_imaged_based
            internal_args['encoder_type'] = 'identity' if symbolic else 'pixel'
            internal_args['max_steps'] = 200
            env_kwargs = {
                'env': args.env_name,
                'symbolic': symbolic,
                'seed': internal_args['seed'],
                'max_episode_length': internal_args['max_steps'],
                'action_repeat': 1,
                'bit_depth': 8,
                'image_dim': None if not_imaged_based else internal_args['env_kwargs']['env_image_size'],
                'env_kwargs': internal_args['env_kwargs'],
                'normalize_observation': False,
                'scale_reward': internal_args['scale_reward'],
                'clip_obs': internal_args['clip_obs'],
                'obs_process': None,
            }
            if internal_args['action_mode']:
                env_kwargs['env_kwargs']['action_mode'] = internal_args['action_mode']
            if internal_args['action_repeat']:
                env_kwargs['env_kwargs']['action_repeat'] = internal_args['action_repeat']
            if internal_args['horizon']:
                env_kwargs['env_kwargs']['horizon'] = internal_args['horizon']
            self.env_kwargs = env_kwargs

            def _instantiate_softgym_env():
                env = SoftGymEnvSB3(**self.env_kwargs)
                return env

            envs = [_instantiate_softgym_env for i in range(args.num_processes)]
