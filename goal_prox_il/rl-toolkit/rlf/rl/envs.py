...
def make_vec_envs(env_name,
    ...
    if use_env_norm:
        if gamma is None:
            print('VecNormalize gamma None')
            envs = VecNormalize(envs, ret=False,
                    ret_raw_obs=alg_env_settings.ret_raw_obs)
        else:
            print('VecNormalize gamma')
    ...
    if env_interface.requires_tensor_wrap():
        ...
        if num_frame_stack is not None and args.frame_stack:
            print('VecPyTorchFrameStack if num_frame_stack is not None and args.frame_stack')
            envs = VecPyTorchFrameStack(envs, num_frame_stack, device)
        elif len(triple_shapes) > 0 and args.frame_stack:
            print('VecPyTorchFrameStack elif len(triple_shapes) > 0 and args.frame_stack')
            envs = VecPyTorchFrameStack(envs, 4, device)

    if (alg_env_settings.state_fn is not None) or (alg_env_settings.action_fn is not None):
        print('EnvNormFnWrapper')
