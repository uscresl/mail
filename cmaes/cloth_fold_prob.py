...
class ClothFoldProb(optim_prob):
    def __init__(self, kwargs):
        ...
        if self.enable_trained_fwd_dyn:
            self._dim_action = 4 # [pick_x, pick_z, place_x, place_z]. We know the best pick_y (-0.9714286) and place_y (-0.71428573).
        ...
        super(ClothFoldProb, self).__init__()
    ...
    def get_actions_learned_fwd_dyn_model(self, actions):
        ...
        for action in actions:
            # We know the best pick_y (-0.9714286) and place_y (-0.71428573)
            full_action = np.array([action[0], -0.9714286, action[1], action[2], -0.71428573, action[3]])
    ...
    def rollout(self, actions, ep_data_teacher, video=False):
        ...
        if self.enable_trained_fwd_dyn:
            ...
            if video:
                self.env.end_record(video_path=f'./data/ClothFold.gif')
                print(f'Saved video!')
        return
