...
class ClothDataset(Dataset):
    ...
    def generate_dataset(self):
        ...
        while rollout_idx < self.n_rollout:
            ...
            self.env.reset()
            if self.env_name == 'DryCloth':
                # need this to allow the plank to show up on screen
                self.env._reset()
            ...
            if self.env_name == 'DryCloth':
                # right most particle (prevent picked particle moving through the wall due to low trajectory)
                # It does not work as well as the middle cloth approach.
                # pp = np.argmax(curr_data['positions'][observable_idx][:, 0])

                # find the middle cloth particle in the x-axis dimension
                num_particles = curr_data['positions'][observable_idx][:, 0].shape[0]
                pp = np.argpartition(curr_data['positions'][observable_idx][:, 0], num_particles // 2)[num_particles // 2]
            else:
    ...
    def _generate_policy_info(self):
        ...
        if self.env_name == 'DryCloth':
            move_direction = np.random.rand(3)
            move_direction[0] = np.random.uniform(-0.8, -0.6)
            move_direction[1] = 0.65
            move_direction[2] = np.random.uniform(-0.5, 0.5)
        else:
    ...
    def load_rollout_data(self, idx_rollout, idx_timestep):
        ...
        initial_data = load_data(self.data_dir, idx_rollout, 0, self.data_names)
        data_cur['gt_reward_crt'] = pc_reward_model(data_cur['positions'][data_cur['downsample_idx']], env_name=self.env_name, initial_pc_pos=initial_data['positions'][data_cur['downsample_idx']])
        return data_cur
    ...
    def prepare_transition(self, idx, eval=False):
        ...
        gt_reward_crt = torch.FloatTensor([pc_reward_model(full_pos_cur[downsample_idx], env_name=self.env_name, initial_pc_pos=initial_data['positions'][downsample_idx])])
        gt_reward_nxt = torch.FloatTensor([pc_reward_model(full_pos_nxt[downsample_idx], env_name=self.env_name, initial_pc_pos=initial_data['positions'][downsample_idx])])
