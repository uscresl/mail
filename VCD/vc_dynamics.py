...
class VCDynamics(object):
    def __init__(self, args, env, vcd_edge=None):
        ...
        if self.args.use_wandb and args.eval == 0:
            ...
            wandb.init(project='deformable-soil', name=args.exp_name, resume='allow',
    ...
    def train(self):
        # Training loop
        # BUG: ↓ should be self.load_epoch+1 since we have already trained load_epoch number of epochs
    ...
    def rollout(self, args):
        ...
        for t in range(H):
            ...
            if picker_pos.shape[0] == 6 or picker_pos.shape[0] == 3:
                # only for DryCloth because it has an extra picker (plank): 6 for training and 3 for evaluation.
                shape_positions[t] = picker_pos[:2]
            else:
            ...
            reward = reward_model(pc_pos, initial_pc_pos=initial_pc_pos, env_name=self.env_name)
            ret += reward
