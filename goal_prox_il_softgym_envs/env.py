...
class SoftGymEnvSB3(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        if self.symbolic:
            ...
            obs = torch.tensor(obs.transpose(2, 0, 1), dtype=torch.uint8)
            return obs
    ...
    def observation_space(self):
        if self._env.observation_mode in ['cam_rgb_key_point', 'depth_key_point']:
            ...
            if self.symbolic:
                ...
                # return Box(low=-np.inf, high=np.inf, shape=(self.image_dim, self.image_dim, self.image_c), dtype=np.float32)
                return Box(low=-np.inf, high=np.inf, shape=(self.image_c, self.image_dim, self.image_dim), dtype=np.float32)
