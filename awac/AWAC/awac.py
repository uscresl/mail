...
class AWAC:
    ...
    def __init__(self, args, env_kwargs):
        ...
        self.ac = actor_critic(self.env.observation_space, self.env.action_space,
                               special_policy=self.awac_policy, bc_model_ckpt_file=bc_model_ckpt_file, env_image_size=self.env_image_size, img_channel=self.img_channel, env_kwargs=env_kwargs, **ac_kwargs)
        ...
        self.save_everything_every_tsteps = args.get('save_everything_every_tsteps', 400000) # about a day and a half using pickerpickandplace action space
    ...
    def eval_agent_five_seeds(self, config):
        ...
        # SOIL
        npy_file_path = "/".join(ckpt_file_path.split('/')[:-1]) + '/eval_five_seeds_results.npy' 
    ...
    def real_image_eval(self, config):
        ...
        from scripts.sim2real.preprocess_realsense_img import preprocess_realsense_image
    ...
    def eval_agent(self, config):
        ...
        for ep in range(num_eval_eps):
            ...
            while ep_len < config['max_steps'] and not done:
                if self.has_image_observations():
                    # visualize DMfD input image
                    # sample_img = obs['image'][0].cpu().detach().numpy()
                    # sample_img = np.transpose(sample_img, (1, 2, 0))
                    # import cv2
                    # sample_img = cv2.cvtColor(sample_img, cv2.COLOR_RGB2BGR)
                    # cv2.imwrite('data/dmfd_input_image.png', sample_img)
    ...
    def test_agent(self, ckpt_path, t):
        ...
        test_ac = core.MLPActorCritic(self.env.observation_space, self.env.action_space,
                               special_policy=self.awac_policy, bc_model_ckpt_file=None, env_image_size=self.env_image_size, img_channel=self.img_channel, env_kwargs=self.env_kwargs, **ac_kwargs)
    ...
    def run(self, args):
        ...
        try:
            for t in range(self.starting_timestep, self.starting_timestep + total_steps):
                ...
                if t != 0 and t % self.save_everything_every_tsteps == 0:
