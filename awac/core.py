...
class awacDrQCNNEncoder(nn.Module):
    def __init__(self, env_image_size, img_channel, feature_dim, env_kwargs):
        super().__init__()
    ...
    def forward(self, obs, detach=False):
        # # DEBUG input image to actor for depth image
        # sample_img = obs[0].cpu().detach().numpy()
        # sample_img = np.transpose(sample_img, (1, 2, 0))
        # import cv2
        # sample_img = cv2.cvtColor(sample_img, cv2.COLOR_RGB2BGR)
        # cv2.normalize(sample_img, sample_img, 0, 255, cv2.NORM_MINMAX)
        # cv2.imwrite('./dmfd_depth_image.png', sample_img)
        # import pdb; pdb.set_trace()
        if self.env_kwargs['env_kwargs']['observation_mode'] == 'cam_rgb_key_point':
            obs = obs / 255.
    ...
    def copy_conv_weights_from(self, source):
        ...
        for i in range(self.num_layers):
            if not isinstance(source.cnn_layers[i], nn.LeakyReLU):
                self.tie_weights(src=source.cnn_layers[i], trg=self.cnn_layers[i])

class awacDrQCNNActor(nn.Module):

    def __init__(self, act_dim, act_limit, env_image_size, img_channel, feature_dim, env_kwargs):
        super().__init__()

        self.encoder = awacDrQCNNEncoder(env_image_size, img_channel, feature_dim, env_kwargs)
...
class awacDrQCNNQFunction(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, env_image_size, img_channel, feature_dim, env_kwargs):
        super().__init__()
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, hidden_sizes=(1024,1024),
                 activation=nn.ReLU, bc_model_ckpt_file=None, special_policy=None, env_image_size=32, env_kwargs=None, img_channel=None):
        super().__init__()
        ...
        if special_policy == 'awac':
            ...
            self.pi = awacDrQCNNActor(act_dim, act_limit, env_image_size, img_channel, feature_dim, env_kwargs).to(device)
        else:
        ...
        if special_policy == 'awac_img':
            self.q1 = awacDrQCNNQFunction(obs_dim, act_dim, env_image_size, img_channel, feature_dim, env_kwargs).to(device)
            self.q2 = awacDrQCNNQFunction(obs_dim, act_dim, env_image_size, img_channel, feature_dim, env_kwargs).to(device)
            self.pi.encoder.copy_conv_weights_from(self.q1.encoder)
            self.q2.encoder.copy_conv_weights_from(self.pi.encoder)
        elif special_policy == 'awac_img_only':
            ...
            self.pi.encoder.copy_conv_weights_from(self.q1.encoder)
            self.q2.encoder.copy_conv_weights_from(self.pi.encoder)
        elif special_policy == 'awac_state':
