...
void DrawCloth(const Vec4* positions, const Vec4* normals, const float* uvs, const int* indices, int numTris, int numPositions, int colorIndex, float expand, bool twosided, bool smooth)
{
	...
	// #### Set both sides to orange color ####
	// colorIndex is orange color; colorIndex+1 is pink color
	// glColor3fv(g_colors[colorIndex+1]*1.5f); // original code: bottom side - pink color
	// glColor3fv(g_colors[colorIndex]*1.5f); // bottom side: orange color
	// glSecondaryColor3fv(g_colors[colorIndex]*1.5f); // top side: orange color
