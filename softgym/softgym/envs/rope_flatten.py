...
class RopeFlattenEnv(RopeNewEnv):
    ...
    def generate_env_variation(self, num_variations=1, config=None, save_to_file=False, **kwargs):
        ...
        for i in range(num_variations):
            ...
            # make sure each configuration has default reward of less than 0.3
            perf = 1.0
            while perf > 0.3:
                random_pick_and_place(pick_num=2, pick_scale=0.005)
                center_object()
                perf = self._get_info_in_numvariations(config)
    ...
    def compute_expert_action(self):
        ...
        if self.action_mode == 'pickerpickandplace':
            ...
            pick1 = particle_pos[0, :3]
            pick2 = particle_pos[-1, :3]
            pick1[1] = 0.01
            pick2[1] = 0.01

            place1 = np.array([-0.2,  0.15, 0.2])
            place2 = np.array([0.2,  0.15, -0.2])
    ...
    def compute_random_actions(self):
        ...
        random_pick_place = np.array([*picked_particle_pos, *place_xyz])
        random_pick_place = self.normalize_action(random_pick_place)
        return random_pick_place
    ...
    def _get_info_in_numvariations(self, config):
        rope_length = config['segment'] * config['radius'] * 0.5
        reward_max = 0
        reward_min = -rope_length
        reward_range = reward_max - reward_min

        curr_endpoint_dist = self._get_endpoint_distance()
        curr_distance_diff = -np.abs(curr_endpoint_dist - rope_length)
