...
class ClothEnv(FlexEnv):
    def __init__(self, observation_mode, action_mode, num_picker=2, render_mode='particle', picker_radius=0.05, picker_threshold=0.005, particle_radius=0.00625, **kwargs):
        ...
        assert observation_mode in ['key_point', 'point_cloud', 'cam_rgb', 'cam_rgb_key_point', 'depth_key_point', 'particles']
        ...
        if action_mode == 'picker':
            ...
            if self.__class__.__name__ == 'ClothFoldEnv':
                ...
                # start using these for DMfD follow up work
                # works for 80x80 cloth size
                picker_low = (-0.5, 0., -0.5)
                picker_high = (0.5, 0.7, 0.5)

                # for 93x93 cloth size
                # picker_low = (-0.8, 0., -0.8)
                # picker_high = (0.8, 0.7, 0.8)
        ...
        if observation_mode in ['key_point', 'point_cloud']:
            ...
            self.observation_space = Box(low=-np.inf, high=np.inf, shape=(self.env_image_size, self.env_image_size, 3),
                                         dtype=np.float32)
        ...
        elif observation_mode == 'particles':
            cloth_dim = 80
            obs_dim = cloth_dim * cloth_dim * 3
            self.observation_space = Box(np.array([-np.inf] * obs_dim), np.array([np.inf] * obs_dim), dtype=np.float32)
    ...
    def _get_obs(self):
        if self.observation_mode in ['cam_rgb_key_point', 'depth_key_point']:
            ...
            if self.observation_mode == 'cam_rgb_key_point':
                ...
                raise NotImplementedError
        elif self.observation_mode == 'particles':
            pos = np.array(pyflex.get_positions()).reshape([-1, 4])[:, :3].flatten()
        else:
            if self.observation_mode == 'cam_rgb':
                return self.get_image(self.env_image_size, self.env_image_size)
    ...
    def forward_dynamics_model_rollout(self, pred_delta_particle_pos):
        self.video_frames.append(self.render(mode='rgb_array'))
        curr_pos = pyflex.get_positions().reshape((-1, 4))
        curr_pos[:, :3] += pred_delta_particle_pos
        pyflex.set_positions(curr_pos)
        self.video_frames.append(self.render(mode='rgb_array'))
